...
async function sendNotification(channel, alert, user) {
  try {
    ...
    if (alert.geofence) body += `Geofence: ${alert.geofence.name}\n`;
    if (alert.meta) body += `Details: ${JSON.stringify(alert.meta)}\n`;

    let target = null;
    if (channel === 'EMAIL') target = user?.email || process.env.DEFAULT_ALERT_EMAIL;
    else if (channel === 'SMS') target = user?.phone || process.env.DEFAULT_ALERT_PHONE;

    if (!target && channel !== 'CONSOLE') throw new Error(`No ${channel} target configured`);
...
export async function createAlert(payload, userContext = {}) {
  ...
  if (!Object.values(AlertType).includes(type)) throw new ValidationError(`Invalid alert type: ${type}`);
    ...
    (async () => {
      try {
        ...
        if (settings && settings.channels) {
          const channelConfig = typeof settings.channels === 'string' ? JSON.parse(settings.channels) : settings.channels;
          channels = Object.entries(channelConfig).filter(([_, enabled]) => enabled).map(([channel]) => channel.toUpperCase());
        }
  ...
  // Also trigger external notification service (Resend/CallMeBot/Twilio) - best effort
  try {
